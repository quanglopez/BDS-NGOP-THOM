...
import { extractListing } from "@/lib/html-extract";
...
export async function POST(req: NextRequest) {
  ...
  const fetched = await safeFetchPage(url);
  if (!fetched.ok) {
    console.log(`[extract] FAIL ${fetched.reason} ip=${ip} url=${url.slice(0, 120)}`);
    return NextResponse.json({ ok: false, reason: fetched.reason, message: fetched.message }, { status: 422, headers: CORS });
  }

  const listing = extractListing(fetched.html, url);
  const contentType = fetched.status === 200 ? "ok" : "partial";
